...
13. Если пользователь прислал фото, но текст INCI на нем нечитаем (сильно смазан, обрезан, перекрыт жестким бликом), не пытайся угадывать состав. Выдай строго один ответ: "⚠️ <b>К сожалению, текст на фото трудно разобрать.</b> Пожалуйста, сделайте более четкий снимок при хорошем освещении (чтобы текст был в фокусе) или просто скопируйте состав текстом."
...
async def handle_photo_ingredients(message: Message):
    ...
    try:
        ...
        await status_msg.edit_text(
            "❌ <b>Техническая заминка или фото не удалось обработать.</b>\n\n"
            "Возможно, сервера перегружены, либо изображение слишком тяжелое.\n"
            "Попробуйте отправить снимок еще раз (убедитесь, что текст в фокусе) или пришлите состав текстом."
        )
